...
draw()
window.bind(
    "<KeyRelease>", change_direction
)  # when you press on any key and then let go
window.mainloop()  # used for listening to window events like key presses
# window.mainloop() is a blocking call, it will not return until the window is closed
# window.mainloop() is a loop that runs until the window is closed, it is used to keep the window open and listen for events
